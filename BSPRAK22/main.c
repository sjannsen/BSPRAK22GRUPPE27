...
#define DATABASESEGMENTSIZE sizeof(int)
...
int delKeyValue(keyValuePair* keyValueStore, char* key, int* SUBDatabase);
int checkForSuscribers(keyValuePair* keyValueStore, char* key, int* SUBDatabase);
...
int main(){
    ...
    databaseSEGMENTID = shmget(IPC_PRIVATE, 10*DATABASESEGMENTSIZE, IPC_CREAT | 0600);
    if (databaseSEGMENTID == -1) {
        perror("[-]Error while getting shared memory database segment\n");
    }

    databaseSEGMENT = (int*) shmat(databaseSEGMENTID, 0, 0);
    if (databaseSEGMENT == (void*) -1) {
        perror("[-]Error while attaching shared memory database segment\n");
    }

    if ((n = shmctl(databaseSEGMENTID, IPC_RMID, 0)) == -1) {
        perror("[-]Error while destroying shared memory database segment\n");
    }


    printf("Key\tValue\n");
    keyValuePair* runPointer = sharedMemorySegment;
    for (int i = 0; i < 10; i++) {
        strcpy(runPointer->key, "0");
        strcpy(runPointer->value, "0");
        printf("%s\t%s\n", runPointer->key, runPointer->value);
        runPointer++;
    }

    printf("SUB Database\n");
    int* databaseRunPointer = databaseSEGMENT;
    for (int i = 0; i < 10; i++) {
        *databaseRunPointer = 0;
        printf("%d\t", *databaseRunPointer);
        databaseRunPointer++;
    }
    printf("\n");
    ...
    semctl(sem_id, 1, SETVAL, 0);  // auf 0

    unsigned short test[1]; //Ausgabe des Semaphors
    ...
    */
    ...
    int subsNeedsToBeIncrementet = 0;
    int subsNeedsToBeDecrementet = 0;
    ...
    */
                if((bytes_read = recv(newSocket, buffer, 1024, 0)) > 0) {
                    ...
                    if(strcmp(buffer, "QUIT") == 0){
                        ...
                        printKeyValueStore(sharedMemorySegment, databaseSEGMENT);
                        ...
                        switch (commandSelected)
                        {
                            ...
                            case 1:
                                ...
                                if ((n = putKeyValue(sharedMemorySegment, keyPart, valuePart)) == -1) {
                                    ...
                                    // SUB CHECK
                                    indexOfSuscriber = checkForSuscribers(sharedMemorySegment, keyPart, databaseSEGMENT);
                                    if (indexOfSuscriber == -1) {
                                        ...
                                        message.mtype = (long) databaseSEGMENT[indexOfSuscriber];
                            ...
                            case 3:
                                ...
                                if ((n = delKeyValue(sharedMemorySegment, keyPart, databaseSEGMENT)) == -1) {
                                    printf("KEY_NOT_FOUND\n");
                                    strcpy(outputBuffer, "DEL:");
                                    strcat(outputBuffer, keyPart);
                                    strcat(outputBuffer, ":key_nonexistent");
                            ...
                            case 8:
                                ...
                                if ((n = getKeyValue(sharedMemorySegment, keyPart)) == -1) {
                                    ...
                                    runPointer = sharedMemorySegment;
                                    for (int i = 0; i < n; i++) {
                                        runPointer++;
                                    }

                                    printf("GET to SUB:%s:%s\n", keyPart, runPointer->value);
                                    strcpy(outputBuffer, "SUB:");
                                    strcat(outputBuffer, keyPart);
                                    strcat(outputBuffer, ":");
                                    strcat(outputBuffer, runPointer->value);

                                    databaseSEGMENT[n] = newSocket;
                                    printf("Added socket %d to suscribers %d\n", newSocket, databaseSEGMENT[n]);
                                    subsNeedsToBeIncrementet = 1;
                        ...
                        if (subsRunning == 1) {
                            if (msgrcv(messageQueueID, &message, sizeof(message), (long) newSocket, 0) == -1) {
                                ...
                                if ((n = getKeyValue(sharedMemorySegment, message.mMessage)) == -1) {
                                    ...
                                    if (strcmp(runPointer->value, "0") != 0) {
                                        printf("SUB Key value was changed\n");
                                        strcpy(outputBuffer, "PUT:");
                                        strcat(outputBuffer, runPointer->key);
                                        strcat(outputBuffer, ":");
                                        strcat(outputBuffer, runPointer->value);
                                    }
                                }
                        ...
                        //////////////////////////////////////
                        printf("SEND: [%s]\n", outputBuffer);
                        if (alreadySent == 0 && commandSelected != 10)
                        ...
                        if (subsNeedsToBeIncrementet == 1) {
                            subsRunning++;
                            subsNeedsToBeIncrementet = 0;
                        }

                        if (subsNeedsToBeDecrementet == 1) {
                            subsRunning--;
                            subsNeedsToBeDecrementet = 0;
                        }
...
int selectCommand(char* commandPart) {
    ...
    else if (strcmp(commandPart, "EMPTY") == 0) {
        return 10;
    }
...
int putKeyValue(keyValuePair* keyValueStore, char* key, char* value) {
    ...
    for (int i = 0; i < 10; i++) {
        if (strcmp(runPointer->key, key) == 0 || strcmp(keyValueStore->key + i*sizeof(keyValuePair), "0") == 0) {
            strcpy(runPointer->key, key);
            strcpy(runPointer->value, value);
            return i;
        }
        runPointer++;
    }
...
int getKeyValue(keyValuePair* keyValueStore, char* key) {
    ...
    for (int i = 0; i < 10; i++) {
        if (strcmp(runPointer->key, key) == 0) {
            return i;
        }
        runPointer++;
    }
...
int delKeyValue(keyValuePair* keyValueStore, char* key, int* SUBDatabase) {
    keyValuePair* runPointer = keyValueStore;
    int* databaseRunPointer = SUBDatabase;
...
int checkForSuscribers(keyValuePair* keyValueStore, char* key, int* SUBDatabase) {
    ...
    if (SUBDatabase[index] > 0) {
        printf("Suscriber socket found %d\n", SUBDatabase[index]);
        return index;
    }
